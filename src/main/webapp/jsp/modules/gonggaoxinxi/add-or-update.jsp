...
<body>
						...
						<div class="widget-area-2 lochana-box-shadow">
							...
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>简介</label>
										<input name="jianjie" id="jianjie" name="jianjie" class="form-control" placeholder="简介">
									</div>
									<div class="form-group col-md-6">
										<label>发布时间</label>
										<div id="fabushijianDate" class="input-append date form_datetime">
											...
											<div class="form-group  col-md-12" >
												<label>详情介绍</label>
												<div><div class="upload"><input id="xiangqingjieshaoupload" name="file" type="file">选择文件</div>
												<script id="xiangqingjieshaoEditor" type="text/plain"></script>
	...
	<script>
		...
		function upload(){
			...
			$('#xiangqingjieshaoupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('xiangqingjieshaoEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
		...
		function setContent(){
			if(ruleForm.xiangqingjieshao != null && ruleForm.xiangqingjieshao != 'null' && ruleForm.xiangqingjieshao != ''){
				var xiangqingjieshaoMes = '' + ruleForm.xiangqingjieshao;
				var xiangqingjieshaoUeditor = UE.getEditor('xiangqingjieshaoEditor');
				xiangqingjieshaoUeditor.ready(function() {
					xiangqingjieshaoUeditor.setContent(xiangqingjieshaoMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('xiangqingjieshaoEditor').hasContents()){
				$('#xiangqingjieshao-input').attr('value',UE.getEditor('xiangqingjieshaoEditor').getContent());
			}
		}
		...
		function dataBind(){
			...
			$("#xiangqingjieshao").val(ruleForm.xiangqingjieshao);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.xiangqingjieshao != null && res.data.xiangqingjieshao != ''){

						$("#xiangqingjieshao").val(res.data.xiangqingjieshao);
                        var xiangqingjieshaoMes = '' + res.data.xiangqingjieshao;
                        var xiangqingjieshaoUeditor = UE.getEditor('xiangqingjieshaoEditor');
                        xiangqingjieshaoUeditor.ready(function() {
                            xiangqingjieshaoUeditor.setContent(xiangqingjieshaoMes);
                        });
